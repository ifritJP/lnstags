...
func (self *DBAccess_DBAccess) MapJoin(tableName string,otherTable string,on string,condition LnsAny,limit LnsAny,attrib LnsAny,_func base.Base_queryForm) bool {
    ...
    if condition != nil{
        condition_115 := condition.(string)
        query = Lns_getVM().String_format("%s WHERE %s", []LnsAny{query, condition_115})
        
    }
    if limit != nil{
        limit_117 := limit.(LnsInt)
        query = Lns_getVM().String_format("%s LIMIT %d", []LnsAny{query, limit_117})
...
func (self *DBAccess_DBAccess) MapJoin2(tableName string,otherTable string,on string,otherTable2 string,on2 string,condition LnsAny,limit LnsAny,attrib LnsAny,_func base.Base_queryForm) bool {
    ...
    if condition != nil{
        condition_133 := condition.(string)
        query = Lns_getVM().String_format("%s WHERE %s", []LnsAny{query, condition_133})
        
    }
    if limit != nil{
        limit_135 := limit.(LnsInt)
        query = Lns_getVM().String_format("%s LIMIT %d", []LnsAny{query, limit_135})
...
func (self *DBAccess_DBAccess) MapJoin3(tableName string,otherTable string,on string,otherTable2 string,on2 string,otherTable3 string,on3 string,condition LnsAny,limit LnsAny,attrib LnsAny,_func base.Base_queryForm) bool {
    ...
    if condition != nil{
        condition_153 := condition.(string)
        query = Lns_getVM().String_format("%s WHERE %s", []LnsAny{query, condition_153})
        
    }
    if limit != nil{
        limit_155 := limit.(LnsInt)
        query = Lns_getVM().String_format("%s LIMIT %d", []LnsAny{query, limit_155})
...
func (self *DBAccess_DBAccess) MapRowList(tableName string,condition LnsAny,limit LnsAny,attrib LnsAny,_func base.Base_queryForm) bool {
    ...
    if condition != nil{
        condition_168 := condition.(string)
        query = Lns_getVM().String_format("SELECT %s FROM %s WHERE %s", []LnsAny{ATTRIB, tableName, condition_168})
    ...
    if limit != nil{
        limit_171 := limit.(LnsInt)
        query = Lns_getVM().String_format("%s LIMIT %d", []LnsAny{query, limit_171})
