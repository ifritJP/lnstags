  ...
  (let ((save (current-buffer))
    ...
    (cond
     ...
     ((or (equal mode "allmut")
	  ...
	  (equal mode "noasync")
	  (equal mode "async")
	  (equal mode "asyncLock"))
      (setq lnstags-opt2 (list mode))
      (setq lnstags-mode "inq")
      (setq select-name mode))
     )
...
(defun lnstags-asyncLock (&optional mode)
  (interactive "P")
  (lnstags-pos-at "asyncLock" 'lnstags-tags-select-mode 'lnstags-tags-select))
(defun lnstags-luaval (&optional mode)
  (interactive "P")
  (lnstags-pos-at "luaval" 'lnstags-tags-select-mode 'lnstags-tags-select))
