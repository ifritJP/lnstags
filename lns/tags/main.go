...
func main_inq_1008_(_env *LnsEnv, inqMode string,pattern string) LnsInt {
    ...
    if _switch139 := inqMode; _switch139 == Option_InqMode__Def {
        Inq_InqDef(_env, db, pattern)
    } else if _switch139 == Option_InqMode__Ref {
        Inq_InqRef(_env, db, pattern, false)
    } else if _switch139 == Option_InqMode__Set {
        Inq_InqRef(_env, db, pattern, true)
    } else if _switch139 == Option_InqMode__AllMut {
        Inq_InqAllmut(_env, db)
    } else if _switch139 == Option_InqMode__Async {
        Inq_InqAsync(_env, db, LnsAst.Ast_Async__Async)
    } else if _switch139 == Option_InqMode__Noasync {
...
// 45: decl @lns.@tags.@main.build
func main_build_1027_(_env *LnsEnv, pathList *LnsList,transCtrlInfo *LnsTypes.Types_TransCtrlInfo) LnsInt {
...
func __main___anonymous_1079_(_env *LnsEnv, item *DBCtrl_ItemNamespace) bool {
    Lns_print([]LnsAny{item.FP.Get_name(_env)})
    return true
}
// 59: decl @lns.@tags.@main.__main
func Main___main(_env *LnsEnv, args *LnsList) LnsInt {
    Lns_main_init( _env )
    var option *Option_Option
    option = Option_analyzeArgs(_env, args)
    if _switch636 := option.FP.Get_mode(_env); _switch636 == Option_Mode__Init {
        DBCtrl_initDB(_env, main_dbPath)
    } else if _switch636 == Option_Mode__Build {
        return main_build_1027_(_env, option.FP.Get_pathList(_env), option.FP.Get_transCtrlInfo(_env))
    } else if _switch636 == Option_Mode__Update {
        var db *DBCtrl_DBCtrl
        ...
        db.FP.Close(_env)
        return main_build_1027_(_env, pathList, option.FP.Get_transCtrlInfo(_env))
    } else if _switch636 == Option_Mode__Suffix {
        var db *DBCtrl_DBCtrl
        ...
        db.FP.MapNamespaceSuffix(_env, option.FP.Get_pattern(_env), DBCtrl_NameSpaceCallback(__main___anonymous_1079_))
        db.FP.Close(_env)
    } else if _switch636 == Option_Mode__Inq {
        main_inq_1008_(_env, option.FP.Get_inqMode(_env), option.FP.Get_pattern(_env))
    } else if _switch636 == Option_Mode__InqAt {
    ...
    } else if _switch636 == Option_Mode__Dump {
        var db *DBCtrl_DBCtrl
        ...
        db.FP.DumpAsync(_env)
        db.FP.Close(_env)
    } else if _switch636 == Option_Mode__Test {
        DBCtrl_test(_env)
    }
    return 0
}
