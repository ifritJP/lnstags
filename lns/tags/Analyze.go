...
// for 69
func Analyze_convExp366(arg1 []LnsAny) LnsInt {
    return Lns_getFromMulti( arg1, 0 ).(LnsInt)
}
// for 83
func Analyze_convExp517(arg1 []LnsAny) LnsInt {
    return Lns_getFromMulti( arg1, 0 ).(LnsInt)
}
// for 92
func Analyze_convExp642(arg1 []LnsAny) LnsInt {
    return Lns_getFromMulti( arg1, 0 ).(LnsInt)
}
// for 113
func Analyze_convExp799(arg1 []LnsAny) LnsInt {
    return Lns_getFromMulti( arg1, 0 ).(LnsInt)
}
// for 174
func Analyze_convExp1116(arg1 []LnsAny) LnsInt {
    return Lns_getFromMulti( arg1, 0 ).(LnsInt)
}
// for 381
func Analyze_convExp2300(arg1 []LnsAny) LnsInt {
    return Lns_getFromMulti( arg1, 0 ).(LnsInt)
}
...
// 402: decl @lns.@tags.@Analyze.dumpRoot
func Analyze_dumpRoot_1090_(rootNode *Nodes.Nodes_RootNode,db *DBCtrl_DBCtrl,streamName string) {
    __func__ := "@lns.@tags.@Analyze.dumpRoot"
    Log_log(Log_Level__Log, __func__, 404, Log_CreateMessage(func() string {
        return streamName
    }))
    
    var filter *Analyze_tagFilter
    filter = NewAnalyze_tagFilter(rootNode, db, streamName)
    rootNode.FP.ProcessFilter(&filter.Nodes_Filter, Analyze_Opt2Stem(NewAnalyze_Opt()))
}


// 409: decl @lns.@tags.@Analyze.start
func Analyze_start(db *DBCtrl_DBCtrl,pathList *LnsList,transCtrlInfo *Types.Types_TransCtrlInfo) {
    var set *LnsSet
    set = NewLnsSet([]LnsAny{})
    Ast_buildAst(LnsLog.Log_Level__Log, pathList, nil, nil, true, transCtrlInfo, front.Front_AstCallback(func(ast *TransUnit.TransUnit_ASTInfo) {
        if Lns_op_not(set.Has(ast.FP.Get_streamName())){
            set.Add(ast.FP.Get_streamName())
            {
                _rootNode := Nodes.Nodes_RootNodeDownCastF(ast.FP.Get_node().FP)
                if !Lns_IsNil( _rootNode ) {
                    rootNode := _rootNode.(*Nodes.Nodes_RootNode)
                    db.FP.Begin()
                    Analyze_dumpRoot_1090_(rootNode, db, ast.FP.Get_streamName())
                    db.FP.Commit()
                }
            }
        }
...
type Analyze_tagFilterMtd interface {
    ...
    addSymbolRef(arg1 LnsInt, arg2 *Types.Types_Position, arg3 bool)
    ...
    registerDecl(arg1 *Nodes.Nodes_NodeManager)
...
func NewAnalyze_tagFilter(arg1 *Nodes.Nodes_RootNode, arg2 *DBCtrl_DBCtrl, arg3 string) *Analyze_tagFilter {
...
// 49: DeclConstr
func (self *Analyze_tagFilter) InitAnalyze_tagFilter(rootNode *Nodes.Nodes_RootNode,db *DBCtrl_DBCtrl,streamName string) {
    self.InitNodes_Filter(true, rootNode.FP.Get_moduleTypeInfo(), rootNode.FP.Get_moduleTypeInfo().FP.Get_scope())
    self.moduleTypeInfo = rootNode.FP.Get_moduleTypeInfo()
...
func (self *Analyze_tagFilter) registerType(typeInfo *LnsAst.Ast_TypeInfo)(LnsInt, bool) {
    ...
    parentNsId = Analyze_convExp366(Lns_2DDD(self.FP.registerType(typeInfo.FP.Get_parentInfo())))
    var name string
...
func (self *Analyze_tagFilter) registerSymbol(symbolInfo *LnsAst.Ast_SymbolInfo)(LnsInt, bool) {
    ...
    var parentNsId LnsInt
    parentNsId = Analyze_convExp517(Lns_2DDD(self.FP.registerType(symbolInfo.FP.Get_namespaceTypeInfo())))
    var name string
...
func (self *Analyze_tagFilter) registDeclSym(symbolInfo *LnsAst.Ast_SymbolInfo) LnsInt {
    ...
    var symNsId LnsInt
    symNsId = Analyze_convExp642(Lns_2DDD(self.FP.registerSymbol(symbolInfo)))
...
func (self *Analyze_tagFilter) addSymbolRef(mbrNsId LnsInt,pos *Types.Types_Position,setOp bool) {
    pos = pos.FP.Get_orgPos()
    
    self.db.FP.AddSymbolRef(mbrNsId, self.FP.getFileId(pos.StreamName), pos.LineNo, pos.Column, setOp)
}

// 109: decl @lns.@tags.@Analyze.tagFilter.registerDecl
func (self *Analyze_tagFilter) registerDecl(nodeManager *Nodes.Nodes_NodeManager) {
    ...
    registDeclTypeOp = func(typeInfo *LnsAst.Ast_TypeInfo,pos *Types.Types_Position) LnsInt {
        var nsId LnsInt
        nsId = Analyze_convExp799(Lns_2DDD(self.FP.registerType(typeInfo)))
        pos = pos.FP.Get_orgPos()
    ...
    for _, _workNode := range( nodeManager.FP.GetDeclFormNodeList().Items ) {
        ...
        for _, _altType := range( workNode.FP.Get_expType().FP.Get_itemTypeInfoList().Items ) {
            altType := _altType.(LnsAst.Ast_TypeInfoDownCast).ToAst_TypeInfo()
            registDeclTypeOp(altType, workNode.FP.Get_pos())
        }
    }
    ...
    addMethod = func(methodType *LnsAst.Ast_TypeInfo,pos *Types.Types_Position) LnsInt {
                ...
                if !Lns_IsNil( _scope ) {
                    ...
                    scope.FP.FilterTypeInfoFieldAndIF(false, scope, LnsAst.Ast_ScopeAccess__Normal, LnsAst.Ast_filterForm(func(symbolInfo *LnsAst.Ast_SymbolInfo) bool {
                        if symbolInfo.FP.Get_name() == methodType.FP.Get_rawTxt(){
                            ...
                            superNsId = Analyze_convExp1116(Lns_2DDD(self.FP.registerType(symbolInfo.FP.Get_typeInfo())))
        ...
        for _, _altType := range( methodType.FP.Get_itemTypeInfoList().Items ) {
            altType := _altType.(LnsAst.Ast_TypeInfoDownCast).ToAst_TypeInfo()
    ...
    for _, _workNode := range( nodeManager.FP.GetDeclAlgeNodeList().Items ) {
        ...
        registDeclType(&workNode.Nodes_Node)
        {
            __collection1272 := workNode.FP.Get_algeType().FP.Get_valInfoMap()
            __sorted1272 := __collection1272.CreateKeyListStr()
            __sorted1272.Sort( LnsItemKindStr, nil )
            for _, _name := range( __sorted1272.Items ) {
                name := _name.(string)
                {
                    __exp := Lns_GetEnv().NilAccFin(Lns_GetEnv().NilAccPush(workNode.FP.Get_algeType().FP.Get_scope()) && 
                    Lns_NilAccCall1( Lns_GetEnv(), func () LnsAny { return Lns_GetEnv().NilAccPop().(*LnsAst.Ast_Scope).FP.GetSymbolInfoChild(name)})/* 201:14 */)
                    if !Lns_IsNil( __exp ) {
                        _exp := __exp.(*LnsAst.Ast_SymbolInfo)
                        self.FP.registDeclSym(_exp)
                    }
    ...
    for _, _workNode := range( nodeManager.FP.GetAliasNodeList().Items ) {
        workNode := _workNode.(Nodes.Nodes_AliasNodeDownCast).ToNodes_AliasNode()
        registDeclTypeOp(workNode.FP.Get_expType(), Lns_unwrap( workNode.FP.Get_newSymbol().FP.Get_pos()).(*Types.Types_Position))
    }
    for _, _workNode := range( nodeManager.FP.GetImportNodeList().Items ) {
        workNode := _workNode.(Nodes.Nodes_ImportNodeDownCast).ToNodes_ImportNode()
        self.FP.registDeclSym(workNode.FP.Get_symbolInfo())
    }
    for _, _workNode := range( nodeManager.FP.GetDeclVarNodeList().Items ) {
        ...
        for _, _symbolInfo := range( workNode.FP.Get_symbolInfoList().Items ) {
            symbolInfo := _symbolInfo.(LnsAst.Ast_SymbolInfoDownCast).ToAst_SymbolInfo()
            if symbolInfo.FP.Get_scope() == self.moduleTypeInfo.FP.Get_scope(){
    ...
    for _, _workNode := range( nodeManager.FP.GetDeclMemberNodeList().Items ) {
        ...
        if workNode.FP.Get_getterMode() != LnsAst.Ast_AccessMode__None{
            ...
            Lns_NilAccCall1( Lns_GetEnv(), func () LnsAny { return Lns_GetEnv().NilAccPop().(*LnsAst.Ast_Scope).FP.GetTypeInfoChild(name)})/* 233:20 */)).(*LnsAst.Ast_TypeInfo), pos)
            self.FP.addSymbolRef(mbrNsId, pos, true)
        }
        if workNode.FP.Get_setterMode() != LnsAst.Ast_AccessMode__None{
            ...
            Lns_GetEnv().NilAccPush(Lns_GetEnv().NilAccPop().(*Types.Types_Token).Pos))).(*Types.Types_Position)
            addMethod(Lns_unwrap( Lns_GetEnv().NilAccFin(Lns_GetEnv().NilAccPush(workNode.FP.Get_classType().FP.Get_scope()) && 
            Lns_NilAccCall1( Lns_GetEnv(), func () LnsAny { return Lns_GetEnv().NilAccPop().(*LnsAst.Ast_Scope).FP.GetTypeInfoChild(name)})/* 241:20 */)).(*LnsAst.Ast_TypeInfo), pos)
            self.FP.addSymbolRef(mbrNsId, pos, false)
        }
    }
}

// 248: decl @lns.@tags.@Analyze.tagFilter.registerRefs
func (self *Analyze_tagFilter) registerRefs(nodeManager *Nodes.Nodes_NodeManager) {
    ...
    var addSymbolRef func(symbolInfo *LnsAst.Ast_SymbolInfo,pos *Types.Types_Position,setOp bool)
    addSymbolRef = func(symbolInfo *LnsAst.Ast_SymbolInfo,pos *Types.Types_Position,setOp bool) {
        __func__ := "@lns.@tags.@Analyze.tagFilter.registerRefs.addSymbolRef"
        if _switch1520 := symbolInfo.FP.Get_name(); _switch1520 == "__func__" || _switch1520 == "__mod__" || _switch1520 == "__line__" {
            return 
        }
        var nsId LnsInt
        var added bool
        ...
        if Lns_isCondTrue( Lns_GetEnv().PopVal( Lns_GetEnv().IncStack() ||
            ...
            Lns_GetEnv().SetStackVal( Lns_op_not(LnsAst.Ast_isBuiltin(symbolInfo.FP.Get_namespaceTypeInfo().FP.Get_typeId().Id))) ).(bool)){
            Log_log(Log_Level__Err, __func__, 258, Log_CreateMessage(func() string {
        ...
        self.FP.addSymbolRef(nsId, pos, setOp)
    }
    ...
    registerRefType = func(typeInfo *LnsAst.Ast_TypeInfo,pos *Types.Types_Position) {
        ...
        if Lns_isCondTrue( Lns_GetEnv().PopVal( Lns_GetEnv().IncStack() ||
            Lns_GetEnv().SetStackVal( added) &&
            Lns_GetEnv().SetStackVal( Lns_op_not(LnsAst.Ast_isBuiltin(typeInfo.FP.Get_typeId().Id))) ).(bool)){
            Log_log(Log_Level__Err, __func__, 267, Log_CreateMessage(func() string {
                return Lns_getVM().String_format("no register type -- %d:%d:%s", []LnsAny{pos.LineNo, pos.Column, self.FP.GetFull(typeInfo, false)})
            }))
            
        }
        self.FP.addSymbolRef(nsId, pos, true)
    }
    var registerRefSym func(symbolInfo *LnsAst.Ast_SymbolInfo,pos *Types.Types_Position,setOp bool)
    registerRefSym = func(symbolInfo *LnsAst.Ast_SymbolInfo,pos *Types.Types_Position,setOp bool) {
        if _switch1917 := symbolInfo.FP.Get_namespaceTypeInfo().FP.Get_kind(); _switch1917 == LnsAst.Ast_TypeInfoKind__Enum || _switch1917 == LnsAst.Ast_TypeInfoKind__Alge {
            addSymbolRef(symbolInfo, pos, true)
        } else {
            if _switch1915 := symbolInfo.FP.Get_kind(); _switch1915 == LnsAst.Ast_SymbolKind__Fun || _switch1915 == LnsAst.Ast_SymbolKind__Mac || _switch1915 == LnsAst.Ast_SymbolKind__Mtd {
                registerRefType(symbolInfo.FP.Get_typeInfo(), pos)
            } else if _switch1915 == LnsAst.Ast_SymbolKind__Typ {
                if symbolInfo.FP.Get_typeInfo().FP.Get_kind() == LnsAst.Ast_TypeInfoKind__Module{
                    addSymbolRef(symbolInfo, pos, true)
                }
                registerRefType(symbolInfo.FP.Get_typeInfo(), pos)
            } else if _switch1915 == LnsAst.Ast_SymbolKind__Mbr {
                addSymbolRef(symbolInfo, pos, setOp)
            } else if _switch1915 == LnsAst.Ast_SymbolKind__Var {
                if Lns_GetEnv().PopVal( Lns_GetEnv().IncStack() ||
                    Lns_GetEnv().SetStackVal( LnsAst.Ast_isPubToExternal(symbolInfo.FP.Get_accessMode())) ||
                    Lns_GetEnv().SetStackVal( symbolInfo.FP.Get_scope() == self.moduleTypeInfo.FP.Get_scope()) ).(bool){
                    addSymbolRef(symbolInfo, pos, setOp)
                }
            } else if _switch1915 == LnsAst.Ast_SymbolKind__Arg {
            }
        }
    }
    for _, _workNode := range( nodeManager.FP.GetExpSetValNodeList().Items ) {
        workNode := _workNode.(Nodes.Nodes_ExpSetValNodeDownCast).ToNodes_ExpSetValNode()
        for _, _leftSym := range( workNode.FP.Get_LeftSymList().Items ) {
            leftSym := _leftSym.(LnsAst.Ast_SymbolInfoDownCast).ToAst_SymbolInfo()
            if _switch1982 := leftSym.FP.Get_kind(); _switch1982 == LnsAst.Ast_SymbolKind__Mbr {
                registerRefSym(leftSym, workNode.FP.Get_pos(), true)
            } else if _switch1982 == LnsAst.Ast_SymbolKind__Var {
                if Lns_GetEnv().PopVal( Lns_GetEnv().IncStack() ||
                    Lns_GetEnv().SetStackVal( LnsAst.Ast_isPubToExternal(leftSym.FP.Get_accessMode())) ||
                    Lns_GetEnv().SetStackVal( leftSym.FP.Get_scope() == self.moduleTypeInfo.FP.Get_scope()) ).(bool){
                    registerRefSym(leftSym, workNode.FP.Get_pos(), true)
                }
            }
        }
    ...
    for _, _workNode := range( nodeManager.FP.GetExpRefNodeList().Items ) {
        ...
        registerRefSym(workNode.FP.Get_symbolInfo(), workNode.FP.Get_pos(), false)
    }
    for _, _workNode := range( nodeManager.FP.GetRefFieldNodeList().Items ) {
        workNode := _workNode.(Nodes.Nodes_RefFieldNodeDownCast).ToNodes_RefFieldNode()
        {
            ...
            if !Lns_IsNil( __exp ) {
                _exp := __exp.(*LnsAst.Ast_SymbolInfo)
                registerRefSym(_exp, workNode.FP.Get_pos(), false)
            } else {
                if Lns_op_not(workNode.FP.Get_macroArgFlag()){
                    Log_log(Log_Level__Warn, __func__, 352, Log_CreateMessage(func() string {
                        return Lns_getVM().String_format("no symbolInfo -- %s, %s:%d:%d", []LnsAny{workNode.FP.Get_field().Txt, workNode.FP.Get_pos().StreamName, workNode.FP.Get_pos().LineNo, workNode.FP.Get_pos().Column})
                    }))
    ...
    for _, _workNode := range( nodeManager.FP.GetExpOmitEnumNodeList().Items ) {
        workNode := _workNode.(Nodes.Nodes_ExpOmitEnumNodeDownCast).ToNodes_ExpOmitEnumNode()
        {
            ...
            Lns_NilAccCall1( Lns_GetEnv(), func () LnsAny { return Lns_GetEnv().NilAccPop().(*LnsAst.Ast_Scope).FP.GetSymbolInfoChild(workNode.FP.Get_valInfo().FP.Get_name())})/* 362:11 */)
            if !Lns_IsNil( __exp ) {
                _exp := __exp.(*LnsAst.Ast_SymbolInfo)
                registerRefSym(_exp, workNode.FP.Get_pos(), false)
            }
    ...
    for _, _workNode := range( nodeManager.FP.GetMatchNodeList().Items ) {
        ...
        for _, _matchCase := range( workNode.FP.Get_caseList().Items ) {
            ...
            valInfo = matchCase.FP.Get_valInfo()
            {
                ...
                Lns_NilAccCall1( Lns_GetEnv(), func () LnsAny { return Lns_GetEnv().NilAccPop().(*LnsAst.Ast_Scope).FP.GetSymbolInfoChild(valInfo.FP.Get_name())})/* 370:14 */)
                if !Lns_IsNil( __exp ) {
                    _exp := __exp.(*LnsAst.Ast_SymbolInfo)
                    registerRefSym(_exp, matchCase.FP.Get_block().FP.Get_pos(), false)
                }
            }
        }
    }
...
// 377: decl @lns.@tags.@Analyze.tagFilter.processRoot
func (self *Analyze_tagFilter) ProcessRoot(node *Nodes.Nodes_RootNode,_opt LnsAny) {
    ...
    var modNsId LnsInt
    modNsId = Analyze_convExp2300(Lns_2DDD(self.FP.registerType(node.FP.Get_moduleTypeInfo())))
    var fileId LnsInt
    fileId = self.FP.getFileId(self.streamName)
    self.db.FP.AddSymbolDecl(modNsId, fileId, 1, 1)
    var projDir string
    ...
    for _, _workNode := range( node.FP.Get_nodeManager().FP.GetSubfileNodeList().Items ) {
        workNode := _workNode.(Nodes.Nodes_SubfileNodeDownCast).ToNodes_SubfileNode()
        {
            ...
            if !Lns_IsNil( _usePath ) {
                ...
                self.db.FP.AddSubfile(fileId, subId)
            }
        }
    }
    self.FP.registerDecl(node.FP.Get_nodeManager())
